...
int Cserver::run() {
    ...
    BOOST_FOREACH(Cconfig::t_listen_ipas::const_reference j, m_config.m_listen_ipas) {
                    BOOST_FOREACH(Cconfig::t_listen_ports::const_reference i, m_config.m_listen_ports) {
                                    ...
                                    if (l.open(SOCK_STREAM) == INVALID_SOCKET)
                                        std::cerr << "socket failed: " << Csocket::error2a(WSAGetLastError())
                                                  << std::endl;
                                    else if (l.setsockopt(SOL_SOCKET, SO_REUSEADDR, true),
                                            l.bind(j, htons(i)))
                                        std::cerr << "bind failed: " << Csocket::error2a(WSAGetLastError())
                                                  << std::endl;
                                    else if (l.listen())
                                        std::cerr << "listen failed: " << Csocket::error2a(WSAGetLastError())
                                                  << std::endl;
                                    else {
                                        ...
                                        if (!m_epoll.ctl(EPOLL_CTL_ADD, l,
                    ...
                    BOOST_FOREACH(Cconfig::t_listen_ports::const_reference i, m_config.m_listen_ports) {
                                    ...
                                    if (l.open(SOCK_DGRAM) == INVALID_SOCKET)
                                        ...
                                        if (!m_epoll.ctl(EPOLL_CTL_ADD, l, EPOLLIN | EPOLLPRI | EPOLLERR | EPOLLHUP,
                                                         &lu.back()))
                                            continue;
                                    }
                                    return 1;
    ...
    while (!g_sig_term) {
        ...
        int n = 0;
        BOOST_FOREACH(t_connections::reference
                              i, m_connections) {
                        int z = i.pre_select(&fd_read_set, &fd_write_set);
                        n = std::max(n, z);
                    }
        BOOST_FOREACH(t_tcp_sockets::reference
                              i, lt) {
                        FD_SET(i.s(), &fd_read_set);
                        n = std::max<int>(n, i.s());
                    }
        BOOST_FOREACH(t_udp_sockets::reference
                              i, lu) {
                        FD_SET(i.s(), &fd_read_set);
                        n = std::max<int>(n, i.s());
                    }
...
void Cserver::accept(const Csocket &l) {
    ...
    while (1) {
        ...
        if (connection->s() != INVALID_SOCKET) {
            ...
            m_epoll.ctl(EPOLL_CTL_ADD, m_connections.back().s(),
                        EPOLLIN | EPOLLOUT | EPOLLPRI | EPOLLERR | EPOLLHUP | EPOLLET, &m_connections.back());
        }
    }
}
...
std::string Cserver::insert_peer(const Ctracker_input &v, bool udp, t_user *user) {
    if (m_use_sql && m_config.m_log_announce) {
        m_announce_log_buffer += Csql_query(m_database, "(?,?,?,?,?,?,?,?,?,?),")
                .p(ntohl(v.m_ipa)).p(ntohs(v.m_port)).p(v.m_event).p(v.m_info_hash).p(v.m_peer_id).p(v.m_downloaded).p(
                v.m_left).p(v.m_uploaded).p(user ? user->uid : 0).p(time()).read();
    }
    ...
    if (i) {
        ...
        BOOST_FOREACH(t_peers::reference
                              j, file.peers) c += j.second.left && j.second.uid == user->uid;
...
std::string Cserver::t_file::select_peers(const Ctracker_input &ti) const {
    ...
    BOOST_FOREACH(t_peers::const_reference
                          i, peers) {
    ...
    if (candidates.size() > c) {
        ...
        BOOST_FOREACH(t_candidates::reference
                              i, candidates) d.append(i.begin(), i.end());
    }
    return d;
}
...
Cvirtual_binary Cserver::select_peers(const Ctracker_input &ti) const {
    ...
    return Cvirtual_binary(
            (boost::format("d8:completei%de10:incompletei%de8:intervali%de12:min intervali%de5:peers%d:%se")
             % f->seeders % f->leechers % config().m_announce_interval % config().m_announce_interval % peers.size() %
             peers).str());
}
...
void Cserver::t_file::clean_up(time_t t, Cserver &server) {
    for (t_peers::iterator i = peers.begin(); i != peers.end();) {
        if (i->second.mtime < t) {
            ...
            if (i->second.uid)
                server.m_files_users_updates_buffer += Csql_query(server.m_database, "(0,0,0,0,-1,0,-1,?,?),").p(fid).p(
                        i->second.uid).read();
            peers.erase(i++);
            dirty = true;
        } else
...
void Cserver::clean_up() {
    BOOST_FOREACH(t_files::reference
                          i, m_files) i.second.clean_up(time() - static_cast<int>(1.5 * m_config.m_announce_interval),
                                                        *this);
...
Cvirtual_binary Cserver::scrape(const Ctracker_input &ti, t_user *user) {
    ...
    if (ti.m_info_hashes.empty()) {
        ...
        BOOST_FOREACH(t_files::reference
                              i, m_files) {
                        if (i.second.leechers || i.second.seeders)
                            d += (boost::format("20:%sd8:completei%de10:downloadedi%de10:incompletei%dee") % i.first %
                                  i.second.seeders % i.second.completed % i.second.leechers).str();
                    }
    } else {
        ...
        BOOST_FOREACH(Ctracker_input::t_info_hashes::const_reference j, ti.m_info_hashes) {
                        if (t_file *i = find_ptr(m_files, j))
                            d += (boost::format("20:%sd8:completei%de10:downloadedi%de10:incompletei%dee") % j %
                                  i->seeders % i->completed % i->leechers).str();
                    }
    }
    d += "e";
...
void Cserver::read_db_deny_from_hosts() {
    ...
    try {
        ...
        BOOST_FOREACH(t_deny_from_hosts::reference
                              i, m_deny_from_hosts) i.second.marked = true;
...
void Cserver::read_db_files_sql() {
    try {
        ...
        Csql_result result = Csql_query(m_database,
                                        "select info_hash, @completed, @fid, ctime from @files where @fid >= ?").p(
                m_fid_end).execute();
        for (Csql_row row; (row = result.fetch_row());) {
...
void Cserver::read_db_users() {
    ...
    try {
        ...
        BOOST_FOREACH(t_users::reference
                              i, m_users) i.second.marked = true;
        m_users_torrent_passes.clear();
        for (Csql_row row; (row = result.fetch_row());) {
            t_user &user = m_users[row[0].i()];
            user.marked = false;
            int c = 0;
            user.uid = (int) row[c++].i();
            if (m_read_users_can_leech)
                user.can_leech = (bool) row[c++].i();
            if (m_read_users_peers_limit)
                user.peers_limit = (int) row[c++].i();
            if (m_read_users_torrent_pass) {
                if (row[c].size())
                    m_users_torrent_passes[row[c].s()] = &user;
                c++;
            }
            user.torrent_pass_version = (int) row[c++].i();
            if (m_read_users_torrents_limit)
                user.torrents_limit = (int) row[c++].i();
            if (m_read_users_wait_time)
                user.wait_time = (int) row[c++].i();
        }
...
void Cserver::write_db_files() {
    ...
    try {
        ...
        BOOST_FOREACH(t_files::reference
                        ...
                        if (!file.dirty)
                            continue;
                        if (!file.fid) {
                            Csql_query(m_database,
                                       "insert into @files (info_hash, mtime, ctime) values (?, unix_timestamp(), unix_timestamp())").p(
                                    i.first).execute();
    ...
    if (!m_announce_log_buffer.empty()) {
        try {
            ...
            m_database.query("insert delayed into " + db_name("announce_log") +
                             " (ipa, port, event, info_hash, peer_id, downloaded, left0, uploaded, uid, mtime) values " +
    ...
    if (!m_scrape_log_buffer.empty()) {
        try {
            ...
            m_database.query("insert delayed into " + db_name("scrape_log") + " (ipa, info_hash, uid, mtime) values " +
                             m_scrape_log_buffer);
        }
        catch (Cdatabase::exception &) {
        }
...
void Cserver::write_db_users() {
    ...
    if (!m_files_users_updates_buffer.empty()) {
        ...
        try {
            m_database.query("insert into " + db_name("files_users") +
                             " (active, announced, completed, downloaded, `left`, uploaded, mtime, fid, uid) values "
    ...
    if (!m_users_updates_buffer.empty()) {
        ...
        try {
            m_database.query(
                    "insert into " + db_name("users") + " (downloaded, uploaded, " + db_name("uid") + ") values "
...
void Cserver::read_config() {
    if (m_use_sql) {
        try {
            Csql_result result = Csql_query(m_database,
                                            "select name, value from @config where value is not null").execute();
            Cconfig config;
            for (Csql_row row; (row = result.fetch_row());) {
            ...
            if (config.m_torrent_pass_private_key.empty()) {
                ...
                Csql_query(m_database, "insert into @config (name, value) values ('torrent_pass_private_key', ?)").p(
                        config.m_torrent_pass_private_key).execute();
            }
            m_config = config;
            ...
            m_database.set_name("deny_from_hosts",
                                m_config.m_table_deny_from_hosts.empty() ? m_table_prefix + "deny_from_hosts"
                                                                         : m_config.m_table_deny_from_hosts);
            m_database.set_name("announce_log", m_config.m_table_announce_log.empty() ? m_table_prefix + "announce_log"
                                                                                      : m_config.m_table_announce_log);
            m_database.set_name("files",
                                m_config.m_table_files.empty() ? m_table_prefix + "files" : m_config.m_table_files);
            m_database.set_name("files_users", m_config.m_table_files_users.empty() ? m_table_prefix + "files_users"
                                                                                    : m_config.m_table_files_users);
            m_database.set_name("scrape_log", m_config.m_table_scrape_log.empty() ? m_table_prefix + "scrape_log"
                                                                                  : m_config.m_table_scrape_log);
            m_database.set_name("users",
                                m_config.m_table_users.empty() ? m_table_prefix + "users" : m_config.m_table_users);
        }
        catch (Cdatabase::exception &) {
        }
    } else {
...
std::string Cserver::debug(const Ctracker_input &ti) const {
    ...
    os
            << "<!DOCTYPE HTML PUBLIC \"-//W3C//DTD HTML 4.01//EN\"><meta http-equiv=refresh content=60><title>XBT Tracker</title>";
    int leechers = 0;
    int seeders = 0;
    int torrents = 0;
    os << "<table>";
    if (ti.m_info_hash.empty()) {
        BOOST_FOREACH(t_files::const_reference
                              i, m_files) {
                        if (!i.second.leechers && !i.second.seeders)
                            continue;
                        ...
                        os << "<tr><td align=right>" << i.second.fid
                           << "<td><a href=\"?info_hash=" << uri_encode(i.first) << "\">" << hex_encode(i.first)
                           << "</a>"
                           << "<td>" << (i.second.dirty ? '*' : ' ')
...
std::string Cserver::statistics() const {
    ...
    if (m_stats.announced()) {
        os << "<tr><td>announced http <td align=right>" << m_stats.announced_http << "<td align=right>"
           << m_stats.announced_http * 100 / m_stats.announced() << " %"
           << "<tr><td>announced udp<td align=right>" << m_stats.announced_udp << "<td align=right>"
           << m_stats.announced_udp * 100 / m_stats.announced() << " %";
    }
    os << "<tr><td>scraped full<td align=right>" << m_stats.scraped_full
       << "<tr><td>scraped<td align=right>" << m_stats.scraped();
    if (m_stats.scraped()) {
        os << "<tr><td>scraped http<td align=right>" << m_stats.scraped_http << "<td align=right>"
           << m_stats.scraped_http * 100 / m_stats.scraped() << " %"
           << "<tr><td>scraped udp<td align=right>" << m_stats.scraped_udp << "<td align=right>"
           << m_stats.scraped_udp * 100 / m_stats.scraped() << " %";
    }
    os << "<tr><td>"
       ...
       << "<tr><td>read config time<td align=right>" << t - m_read_config_time << " / "
       << m_config.m_read_config_interval
       << "<tr><td>clean up time<td align=right>" << t - m_clean_up_time << " / " << m_config.m_clean_up_interval
       << "<tr><td>read db files time<td align=right>" << t - m_read_db_files_time << " / "
       << m_config.m_read_db_interval;
    if (m_use_sql) {
        os << "<tr><td>read db users time<td align=right>" << t - m_read_db_users_time << " / "
           << m_config.m_read_db_interval
           << "<tr><td>write db files time<td align=right>" << t - m_write_db_files_time << " / "
           << m_config.m_write_db_interval
           << "<tr><td>write db users time<td align=right>" << t - m_write_db_users_time << " / "
           << m_config.m_write_db_interval;
    }
    os << "</table>";
    return os.str();
...
Cserver::t_user *Cserver::find_user_by_torrent_pass(const std::string &v, const std::string &info_hash) {
    if (t_user *user = find_user_by_uid(read_int(4, hex_decode(v.substr(0, 8))))) {
        if (v.size() >= 8 &&
            Csha1((boost::format("%s %d %d %s") % m_config.m_torrent_pass_private_key % user->torrent_pass_version %
                   user->uid % info_hash).str()).read().substr(0, 12) == hex_decode(v.substr(8)))
            return user;
    }
...
int Cserver::test_sql() {
    ...
    try {
        ...
        if (m_config.m_log_announce)
            Csql_query(m_database,
                       "select id, ipa, port, event, info_hash, peer_id, downloaded, left0, uploaded, uid, mtime from @announce_log where 0").execute();
        ...
        Csql_query(m_database,
                   "select @fid, info_hash, @leechers, @seeders, flags, mtime, ctime from @files where 0").execute();
        Csql_query(m_database,
                   "select fid, uid, active, announced, completed, downloaded, `left`, uploaded from @files_users where 0").execute();
        ...
        m_read_users_torrents_limit = Csql_query(m_database,
                                                 "show columns from @users like 'torrents_limit'").execute();
